...
**MANDATORY:** v1 = two **pace** bars (Auto / API) from `usage-summary` only – see `@docs/Specs/2026-08-07 – Requirements.md`. No event/model heuristic in v1.
...
macOS menu bar fork of [c-johannesen/cursorbar](https://github.com/c-johannesen/cursorbar). Problem: upstream `D` mixes Auto+API today-spend; goal is transparent per-pool pace so API quota is not burned early in the billing cycle.

## Канон

- `@README.md` – upstream entry + fork pointer
- `@docs/CursorBar Pace – Hub.md` – product hub
- `@docs/Specs/2026-08-07 – Requirements.md` – v1 requirements / acceptance
...
- Do not commit vault contents via `docs/` (gitignore the symlink path if needed).
- v1: pace from `autoPercentUsed` / `apiPercentUsed` + working-day fraction. Defer today-$ split.
...
| `assets/` | upstream screenshot (moved off `docs/`) |
| `docs/` | Symlink → Obsidian product docs |

## Правила работы агента

1. Read `@docs/Specs/2026-08-07 – Requirements.md` before changing product scope.
